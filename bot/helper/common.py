...
class TaskConfig:
    ...
    async def beforeStart(self):
        ...
        if not self.isLeech:
            ...
            if self.upDest:
                if not isinstance(self.upDest, int):
                    if self.upDest.isdigit():
                        ...
                        _, group_id = self.upDest.split("|")
                        try:
                            await self.client.get_group(group_id)
                        except:
                            raise ValueError(
            ...
            self.splitSize = min(self.splitSize, MAX_SPLIT_SIZE)
    ...
    async def run_multi(self, input_list, folder_name, obj):
        ...
        if len(self.bulk) != 0:
            ...
            msgts = " ".join(msg)
            if self.multi > 2:
                msgts += f"\nCancel Multi: <copy>/{BotCommands.CancelTaskCommand[1]} {self.multiTag}</copy>"
            nextmsg = await sendMessage(self.message, msgts)
        else:
            msg = [s.strip() for s in input_list]
            index = msg.index("-i")
            msg[index + 1] = f"{self.multi - 1}"
            nextmsg = await self.client.get_message(
                message_id=self.message.replied_to_id + 1
            )
            msgts = " ".join(msg)
            if self.multi > 2:
                msgts += f"\nCancel Multi: <copy>/{BotCommands.CancelTaskCommand[1]} {self.multiTag}</copy>"
